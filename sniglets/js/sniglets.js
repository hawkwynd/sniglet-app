...
/*
* fetchAlpha
* List all sniglets based on the letter clicked on
*
 */
function fetchAlpha (letter) {
    ...
    $.get('sniglet_edit.php?func=searchByAlpha&letter=' + letter, function(results) {
        ...
        myObj = jQuery.parseJSON(results);
         ...
         string = '<dv class="listing"><p>'+ myObj.length + ' sniglets found starting with the letter "' + letter + '"</p></div>';

        $.each(myObj, function(key, value) {
            string += '<div class="listing"><b>' + value.sniglet_term + '</b> (<i>' + value.sniglet_phonetics + '</i>) ';
...
function list() {
    ...
    var string = '<dv class="listing"><p>Listing all '+ obj.length + ' records.</p></div>';
...
function hideHeader(){$('#header').fadeOut('slow');}
